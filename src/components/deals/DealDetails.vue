    ...
    <div class="bg-white dark:bg-gray-800">
        ...
        <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="text-xl font-semibold text-gray-900 dark:text-white">{{ deal.name }}</h2>
                    <div class="flex items-center mt-1 space-x-4">
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium"
                            :class="stageClasses[deal.stage]">
                            {{ stageLabels[deal.stage] }}
                        </span>
                        <span v-if="deal.value" class="text-lg font-semibold text-green-600 dark:text-green-400">
                ...
                <div class="flex items-center space-x-2">
                    <button @click="showActivityForm = true"
                        class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                        <svg class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                        </svg>
                        Log Activity
                    </button>
                    <button @click="$emit('edit', deal)" class="text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300 p-2">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                        </svg>
                    </button>
                    <button @click="$emit('close')" class="text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300 p-2">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="px-6 py-4 space-y-6">
            ...
            <div v-if="deal.contact">
                <h3 class="text-sm font-medium text-gray-900 dark:text-white mb-3">Contact</h3>
                <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            <p class="text-sm font-medium text-gray-900 dark:text-white">{{ deal.contact.name }}</p>
            ...
            <!-- Deal Information -->
            <div>
                <h3 class="text-sm font-medium text-gray-900 dark:text-white mb-3">Deal Information</h3>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">
                            Expected Close Date
                        </label>
                        <p class="mt-1 text-sm text-gray-900 dark:text-white" :class="closeDateClass">
                            {{ deal.expected_close_date ? formatDate(deal.expected_close_date) : 'Not set' }}
                            <span v-if="isOverdue" class="text-red-600 dark:text-red-400 ml-2">(Overdue)</span>
                        </p>
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">
            ...
            <!-- Notes -->
            <div v-if="deal.notes">
                <h3 class="text-sm font-medium text-gray-900 dark:text-white mb-3">Notes</h3>
                <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
            ...
            <!-- Stage Progress -->
            <div>
                <h3 class="text-sm font-medium text-gray-900 dark:text-white mb-3">Pipeline Progress</h3>
                <div class="space-y-2">
                    <div v-for="stage in pipelineStages" :key="stage.value" class="flex items-center">
                        <div class="flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center"
                            ...
                            <div v-else-if="stage.value === deal.stage" class="w-2 h-2 bg-white dark:bg-gray-800 rounded-full"></div>
                        </div>
                        <div class="ml-3">
                            <p class="text-sm"
                                :class="stage.value === deal.stage ? 'font-medium text-gray-900 dark:text-white' : 'text-gray-500 dark:text-gray-400'">
                                {{ stage.label }}
                            </p>
                        </div>
                    </div>
            ...
            <div>
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-sm font-medium text-gray-900 dark:text-white">Activity Timeline</h3>
                    <button @click="showActivityForm = true"
